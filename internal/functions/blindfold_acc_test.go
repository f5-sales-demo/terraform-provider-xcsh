...
// These tests require:
//   - TF_ACC=1
//   - XCSH_API_URL set to F5XC console URL
//   - Either XCSH_API_TOKEN or (XCSH_P12_FILE + XCSH_P12_PASSWORD)
//
...
func skipIfNoAuth(t *testing.T) {
	...
	if token == "" && p12File == "" {
		t.Skip("Acceptance tests skipped: no authentication configured (XCSH_API_TOKEN or XCSH_P12_FILE)")
	}
}
...
func skipIfNoURL(t *testing.T) {
	...
	if os.Getenv(blindfold.EnvAPIURL) == "" {
		t.Skip("Acceptance tests skipped: XCSH_API_URL not set")
	}
}
...
func TestAccBlindfoldFunction_P12Auth(t *testing.T) {
	...
	if p12File == "" || p12Password == "" {
		t.Skip("Skipping P12 auth test: XCSH_P12_FILE and XCSH_P12_PASSWORD not set")
	}
...
func TestAccBlindfoldFunction_TokenAuth(t *testing.T) {
	...
	if token == "" {
		t.Skip("Skipping token auth test: XCSH_API_TOKEN not set")
	}
