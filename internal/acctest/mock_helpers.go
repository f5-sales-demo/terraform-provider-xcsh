...
var (
	// globalMockServer is a shared mock server for all tests when XCSH_MOCK_MODE is set
...
func SetupMockTest(t *testing.T) *MockTestConfig {
	...
	originalURL := os.Getenv("XCSH_API_URL")
	originalToken := os.Getenv("XCSH_API_TOKEN")

	// Configure environment to point to mock server
	_ = os.Setenv("XCSH_API_URL", server.URL())
	_ = os.Setenv("XCSH_API_TOKEN", "mock-token")
...
func (m *MockTestConfig) Cleanup() {
	...
	if m.OriginalURL != "" {
		_ = os.Setenv("XCSH_API_URL", m.OriginalURL)
	} else {
		_ = os.Unsetenv("XCSH_API_URL")
	}

	if m.OriginalToken != "" {
		_ = os.Setenv("XCSH_API_TOKEN", m.OriginalToken)
	} else {
		_ = os.Unsetenv("XCSH_API_TOKEN")
	}
...
// SkipIfNoMockMode skips the test if XCSH_MOCK_MODE is not set.
// This allows running mock tests only when explicitly requested.
func SkipIfNoMockMode(t *testing.T) {
	t.Helper()

	if os.Getenv("XCSH_MOCK_MODE") == "" {
		t.Skip("Skipping mock test: XCSH_MOCK_MODE not set")
	}
}

// SkipIfRealAPI skips the test if running against the real API (XCSH_MOCK_MODE is not set).
...
func IsMockMode() bool {
	return os.Getenv("XCSH_MOCK_MODE") != ""
}

// GetGlobalMockServer returns the global mock server, creating it if necessary.
// This ensures all tests share the same mock server when XCSH_MOCK_MODE is set.
func GetGlobalMockServer() *mocks.Server {
	globalMockServerOnce.Do(func() {
		...
		originalEnvVars["XCSH_API_URL"] = os.Getenv("XCSH_API_URL")
		originalEnvVars["XCSH_API_TOKEN"] = os.Getenv("XCSH_API_TOKEN")
		originalEnvVars["XCSH_P12_FILE"] = os.Getenv("XCSH_P12_FILE")
		originalEnvVars["XCSH_P12_PASSWORD"] = os.Getenv("XCSH_P12_PASSWORD")
		originalEnvVars["XCSH_CERT"] = os.Getenv("XCSH_CERT")
		originalEnvVars["XCSH_KEY"] = os.Getenv("XCSH_KEY")
		globalMockServerMu.Unlock()

		// Override environment to use mock server
		_ = os.Setenv("XCSH_API_URL", globalMockServer.URL())
		_ = os.Setenv("XCSH_API_TOKEN", "mock-token")
		// Clear P12/PEM credentials so the provider uses token auth
		_ = os.Unsetenv("XCSH_P12_FILE")
		_ = os.Unsetenv("XCSH_P12_PASSWORD")
		_ = os.Unsetenv("XCSH_CERT")
		_ = os.Unsetenv("XCSH_KEY")
	})
	return globalMockServer
}

// EnsureMockModeConfigured ensures that if XCSH_MOCK_MODE is set,
...
// If XCSH_MOCK_MODE is set, uses mock server.
...
func RunWithMockOrReal(t *testing.T, testCase resource.TestCase, mockSetup func(*MockTestConfig)) {
	...
	t.Skip("Skipping: neither TF_ACC with credentials nor XCSH_MOCK_MODE is set")
}
