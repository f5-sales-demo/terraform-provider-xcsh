...
func TestAccApiDiscoveryDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.xcsh_api_discovery.test"
...
data "xcsh_api_discovery" "test" {
  depends_on = [xcsh_api_discovery.test]
  name       = xcsh_api_discovery.test.name
  namespace  = xcsh_api_discovery.test.namespace
}
`, nsName, name))
}
