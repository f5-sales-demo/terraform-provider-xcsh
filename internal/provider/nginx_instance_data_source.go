...
var (
	_ datasource.DataSource              = &NginxInstanceDataSource{}
	_ datasource.DataSourceWithConfigure = &NginxInstanceDataSource{}
)
...
// NginxInstanceEmptyModel represents empty nested blocks
type NginxInstanceEmptyModel struct {
}

// NginxInstanceAPIDiscoverySpecModel represents api_discovery_spec block
type NginxInstanceAPIDiscoverySpecModel struct {
	Disabled types.Object `tfsdk:"disabled"`
	Enabled  types.Object `tfsdk:"enabled"`
}

// NginxInstanceAPIDiscoverySpecModelAttrTypes defines the attribute types for NginxInstanceAPIDiscoverySpecModel
var NginxInstanceAPIDiscoverySpecModelAttrTypes = map[string]attr.Type{
	"disabled": types.ObjectType{AttrTypes: map[string]attr.Type{}},
	"enabled":  types.ObjectType{AttrTypes: map[string]attr.Type{}},
}

// NginxInstanceWAFSpecModel represents waf_spec block
type NginxInstanceWAFSpecModel struct {
...
// NginxInstanceWAFSpecModelAttrTypes defines the attribute types for NginxInstanceWAFSpecModel
var NginxInstanceWAFSpecModelAttrTypes = map[string]attr.Type{
...
type NginxInstanceDataSourceModel struct {
	...
	APIDiscoverySpec *NginxInstanceAPIDiscoverySpecModel `tfsdk:"api_discovery_spec"`
	WAFSpec          *NginxInstanceWAFSpecModel          `tfsdk:"waf_spec"`
}
...
func (d *NginxInstanceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages a Nginx Instance resource in F5 Distributed Cloud for get nginx instance configuration. configuration. (read-only data source)",
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				MarkdownDescription: "Name of the NginxInstance to look up.",
				Required:            true,
			},
			"namespace": schema.StringAttribute{
				MarkdownDescription: "Namespace of the NginxInstance.",
				Required:            true,
			},
			...
			"host_name": schema.StringAttribute{
				MarkdownDescription: "Dataplane identifier name for instance in NGINX One.",
				Computed:            true,
			},
			"object_id": schema.StringAttribute{
				MarkdownDescription: "Identifier for individual instance in NGINX One.",
				Computed:            true,
			},
...
func (d *NginxInstanceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	resource, err := d.client.GetNginxInstance(ctx, data.Namespace.ValueString(), data.Name.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read NginxInstance: %s", err))
		return
	}
	...
	if blockData, ok := apiResource.Spec["api_discovery_spec"].(map[string]interface{}); ok && (isImport || data.APIDiscoverySpec != nil) {
		data.APIDiscoverySpec = &NginxInstanceAPIDiscoverySpecModel{
	...
	if v, ok := apiResource.Spec["host_name"].(string); ok && v != "" {
	...
	if blockData, ok := apiResource.Spec["waf_spec"].(map[string]interface{}); ok && (isImport || data.WAFSpec != nil) {
		data.WAFSpec = &NginxInstanceWAFSpecModel{
