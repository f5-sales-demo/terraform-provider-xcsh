...
-> **One of the following:**
&#x2022; <a id="default-pool"></a>[`default_pool`](#default-pool) - Optional Block<br>Configuration parameter for default pool
...
<a id="l7-ddos-protection"></a>&#x2022; [`l7_ddos_protection`](#l7-ddos-protection) - Optional Block  Defaults to `map[]`<br>L7 DDOS protection is critical for safeguarding web applications, APIs, and services that are exposed to the internet from sophisticated, volumetric, application-level threats. Configure actions, thresholds and policies to apply during L7 DDOS attack.  Server applies default
when omitted
...
[Metadata](#metadata-46451b) below.

<a id="matcher-25d16b"></a>&#x2022; [`request_matcher`](#matcher-25d16b) - Optional Block<br>Configuration parameter for request matcher<br>See [Request Matcher](#matcher-25d16b) below.
...
[Metadata](#metadata-b7fd60) below.

<a id="matcher-58ddde"></a>&#x2022; [`request_matcher`](#matcher-58ddde) - Optional Block<br>Configuration parameter for request matcher<br>See [Request Matcher](#matcher-58ddde) below.
...
<a id="limiter-38a124"></a>&#x2022; [`inline_rate_limiter`](#limiter-38a124) - Optional Block<br>Configuration parameter for inline rate limiter<br>See [Inline Rate Limiter](#limiter-38a124) below.
...
<a id="matcher-869fa1"></a>&#x2022; [`request_matcher`](#matcher-869fa1) - Optional Block<br>Configuration parameter for request matcher<br>See [Request Matcher](#matcher-869fa1) below.
...
<a id="matcher-d0eea8"></a>&#x2022; [`request_matcher`](#matcher-d0eea8) - Optional Block<br>Configuration parameter for request matcher<br>See [Request Matcher](#matcher-d0eea8) below.
...
<a id="token-2a2002"></a>&#x2022; [`bearer_token`](#token-2a2002) - Optional Block<br>Configuration parameter for bearer token<br>See [Bearer Token](#token-2a2002) below.
...
<a id="key-04416f"></a>&#x2022; [`token_response_key`](#key-04416f) - Optional String<br>Specifies the key name used to extract the authentication token from the login response, such as token or access_token
...
<a id="blocked-clients-http-header"></a>&#x2022; [`http_header`](#blocked-clients-http-header) - Optional Block<br>Configuration parameter for HTTP header<br>See [HTTP Header](#blocked-clients-http-header) below.
...
create<br>See [Metadata](#data-guard-rules-metadata) below.
...
during create<br>See [Metadata](#ddos-mitigation-rules-metadata) below.
...
<a id="matcher-d4dd17"></a>&#x2022; [`tls_fingerprint_matcher`](#matcher-d4dd17) - Optional Block<br>TLS fingerprint matcher specifies multiple criteria for matching a TLS fingerprint. The set of supported positive match criteria includes a list of known classes of TLS fingerprints and a list of exact values. The match is considered successful if either of these positive criteria are
satisfied<br>See [TLS Fingerprint Matcher](#matcher-d4dd17) below.
...
<a id="default-pool-loadbalancer-algorithm"></a>&#x2022; [`loadbalancer_algorithm`](#default-pool-loadbalancer-algorithm) - Optional String  Defaults to `ROUND_ROBIN`<br>Possible values are `ROUND_ROBIN`, `LEAST_REQUEST`, `RING_HASH`, `RANDOM`, `LB_OVERRIDE`<br>[Enum: ROUND_ROBIN|LEAST_REQUEST|RING_HASH|RANDOM|LB_OVERRIDE] Different load balancing algorithms supported When a connection to a
...
<a id="service-799005"></a>&#x2022; [`consul_service`](#service-799005) - Optional Block<br>Specify origin server with Hash Corp Consul service name and site information<br>See [Consul Service](#service-799005) below.
...
<a id="default-pool-list-pools-priority"></a>&#x2022; [`priority`](#default-pool-list-pools-priority) - Optional Number<br>Priority of this origin pool, valid only with multiple origin pools. Value of 0 will make the pool as lowest priority origin pool Priority of 1 means highest priority and is considered active. When active origin pool is not available, lower priority origin pools are made
active as per the
...
<a id="default-route-pools-priority"></a>&#x2022; [`priority`](#default-route-pools-priority) - Optional Number<br>Priority of this origin pool, valid only with multiple origin pools. Value of 0 will make the pool as lowest priority origin pool Priority of 1 means highest priority and is considered active. When active origin pool is not available, lower priority origin pools are made active as per
the
...
<a id="https-tls-cert-params"></a>&#x2022; [`tls_cert_params`](#https-tls-cert-params) - Optional Block<br>Configuration parameter for TLS cert params<br>See [TLS Cert Params](#https-tls-cert-params) below.

<a id="https-tls-parameters"></a>&#x2022; [`tls_parameters`](#https-tls-parameters) - Optional Block<br>Configuration parameter for TLS parameters<br>See [TLS Parameters](#https-tls-parameters) below.
...
<a id="jwt-validation-token-location"></a>&#x2022; [`token_location`](#jwt-validation-token-location) - Optional Block<br>Configuration parameter for token location<br>See [Token Location](#jwt-validation-token-location) below.
...
<a id="servers-8872e3"></a>&#x2022; [`authorization_servers`](#servers-8872e3) - Optional Block<br>Authorization Servers are configured separately in the 'Shared Objects' section of the Web App & API Protection workspace and used to fetch JWKS for JWT validation<br>See [Authorization Servers](#servers-8872e3) below.

#### JWT Validation Authorization Server Authorization Servers

An [`authorization_servers`](#servers-8872e3) block (within [`jwt_validation.authorization_server`](#jwt-validation-authorization-server)) supports the following:

<a id="name-606989"></a>&#x2022; [`name`](#name-606989) - Optional String<br>When a configuration object(e.g. Virtual_host) refers to another(e.g route) then name will hold the referred object's(e.g. Route's) name

<a id="namespace-6bb030"></a>&#x2022; [`namespace`](#namespace-6bb030) - Optional String<br>When a configuration object(e.g. Virtual_host) refers to another(e.g route) then namespace will hold the referred object's(e.g. Route's) namespace

<a id="tenant-a7c313"></a>&#x2022; [`tenant`](#tenant-a7c313) - Optional String<br>When a configuration object(e.g. Virtual_host) refers to another(e.g route) then tenant will hold the referred object's(e.g. Route's) tenant
...
<a id="token-e5c0e3"></a>&#x2022; [`bearer_token`](#token-e5c0e3) - Optional Block<br>Configuration parameter for bearer token
...
<a id="challenge-84ab9e"></a>&#x2022; [`clientside_action_captcha_challenge`](#challenge-84ab9e) - Optional Block<br>Enables loadbalancer to perform captcha challenge Captcha challenge will be based on Google Recaptcha. With this feature enabled, only clients that pass the captcha challenge will be allowed to complete the HTTP request. When loadbalancer is configured to do Captcha Challenge, it
will redirect<br>See [Clientside Action Captcha Challenge](#challenge-84ab9e) below.
...
do<br>See [Clientside Action Js Challenge](#challenge-1070c2) below.
...
do<br>See [Mitigation Js Challenge](#challenge-2a2755) below.
...
[Metadata](#metadata-20b73a) below.
...
<a id="more-option-max-request-header-size"></a>&#x2022; [`max_request_header_size`](#more-option-max-request-header-size) - Optional Number<br>The maximum request header size for downstream connections, in KiB. A HTTP 431 (Request Header Fields Too Large) error code is sent for requests that exceed this size. If multiple load balancers share the same advertise_policy, the highest value configured
...
<a id="rules-7a881b"></a>&#x2022; [`origin_server_subset_rules`](#rules-7a881b) - Optional Block<br>Origin Server Subset Rules allow users to define match condition on Client (IP address, ASN, Country), IP Reputation, Regional Edge names, Request for subset selection of origin servers. Origin Server Subset is a sequential engine where rules are evaluated one after the other. It's important
to<br>See [Origin Server Subset Rules](#rules-7a881b) below.
...
[Metadata](#metadata-72ce94) below.

<a id="spec-fbd0f9"></a>&#x2022; [`spec`](#spec-fbd0f9) - Optional Block<br>Challenge Rule consists of an unordered list of predicates and an action. The predicates are evaluated against a set of input fields that are extracted from or derived from an L7 request API. A request API is considered to match the rule if all predicates in the rule evaluate to true for that<br>See [Spec](#spec-fbd0f9)
below.
...
gets<br>See [Cookie](#ring-hash-hash-policy-cookie) below.
...
<a id="policy-2b5b42"></a>&#x2022; [`specific_hash_policy`](#policy-2b5b42) - Optional Block<br>Policy configuration for this feature<br>See [Specific Hash Policy](#policy-2b5b42) below.
...
Socket Config](#config-8876be) below.
...
<a id="trusted-clients-http-header"></a>&#x2022; [`http_header`](#trusted-clients-http-header) - Optional Block<br>Configuration parameter for HTTP header<br>See [HTTP Header](#trusted-clients-http-header) below.
...
[Metadata](#metadata-09584f) below.
