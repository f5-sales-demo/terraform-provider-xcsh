...
func normalizeGeneratedDocSpacing(filePath string) error {
	content, err := os.ReadFile(filePath)
	if err != nil {
		return err
	}

	normalized := docfmt.NormalizeMarkdownSpacing(string(content))
	return os.WriteFile(filePath, []byte(normalized), 0644)
}
...
func transformAnchorsOnly(filePath string, content string) error {
	...
	result = docfmt.InjectSyntaxRulesCallout(result)
...
func insertCommonTypesSection(content, commonTypes string) string {
	return docfmt.InsertSectionBeforeOnce(
		content,
		"## Common Types",
		commonTypes,
		"## Import",
...
func injectVerifiedExamples(docsDir, examplesDir string) error {
	...
	for _, docFile := range docFiles {
		baseName := strings.TrimSuffix(filepath.Base(docFile), ".md")
		resourceDir := filepath.Join(examplesDir, "xcsh_"+baseName)
		...
		for _, f := range files {
			...
			if name == "resource.tf" || name == "data-source.tf" {
				continue
			}
			...
			exampleName = strings.Title(exampleName) //nolint:staticcheck
			...
			for _, line := range lines {
				...
				if !inComment {
					hclLines = append(hclLines, line)
				}
			}
			hcl := strings.TrimSpace(strings.Join(hclLines, "\n"))
			if hcl == "" {
				continue
			}

			verifiedExamples = append(verifiedExamples,
				fmt.Sprintf("### %s example\n\n```hcl\n%s\n```", exampleName, hcl))
		}

		if len(verifiedExamples) == 0 {
			continue
		}
		...
		if strings.Contains(doc, "## Verified Configuration Examples") {
			continue
		}
		...
		section := "\n\n## Verified Configuration Examples\n\n" +
			...
			strings.Join(verifiedExamples, "\n\n")

		insertAfter := "## Example Usage"
		if idx := strings.Index(doc, "## Argument Reference"); idx > 0 {
			doc = doc[:idx] + section + "\n\n" + doc[idx:]
		} else if idx := strings.Index(doc, insertAfter); idx > 0 {
			nextSection := strings.Index(doc[idx+len(insertAfter):], "\n## ")
			if nextSection > 0 {
				insertPos := idx + len(insertAfter) + nextSection
				doc = doc[:insertPos] + section + "\n\n" + doc[insertPos:]
			} else {
				doc = doc + section
			}
		}
