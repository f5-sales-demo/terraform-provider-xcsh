...
func loadV2SpecMetadata() {
	...
	// Provider functions are generated by tfplugindocs but do not use the
	// resource schema transformer. Normalize their Markdown in the same
	// generation pass so checked-in output is reproducible and lint-clean.
	functionFiles, err := filepath.Glob(filepath.Join("docs/functions", "*.md"))
	if err == nil {
		for _, file := range functionFiles {
			if err := normalizeGeneratedDocSpacing(file); err != nil {
