...
func CleanOrphanGeneratedFiles(outDir, clntDir string, generatedFiles map[string]GeneratedFileSet) {
	suffixes := []struct {
		...
		{outDir, "_action.go", func(files GeneratedFileSet) bool { return files.Action }},
...
func GenerateProviderRegistration(results []openapi.GenerationResult, outputDir string) {
	...
	for _, r := range results {
		if r.Success && !added[r.ResourceName] {
			...
			if !r.IsReadOnly && !r.IsTerraformAction {
				resources = append(resources, fmt.Sprintf("\t\tNew%sResource,", titleCase))
			}
			...
			if !r.IsAction && !r.IsTerraformAction {
				dataSources = append(dataSources, fmt.Sprintf("\t\tNew%sDataSource,", titleCase))
			}
			if r.IsTerraformAction {
				actions = append(actions, fmt.Sprintf("\t\tNew%sAction,", titleCase))
			}
			added[r.ResourceName] = true
		}
	}
	...
	fmt.Printf("\n📝 Updating provider.go with %d resources, %d data sources, and %d actions...\n", len(resources), len(dataSources), len(actions))
