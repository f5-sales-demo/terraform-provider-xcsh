...
const syntaxRulesCallout = "-> **Syntax Rule:** This provider uses OneOf groups for mutually " +
	"exclusive options. Fields documented as \"Optional Block\" use block syntax " +
	"`field_name { ... }`. Empty OneOf object attributes use `field_name = {}`; " +
	"conditional selection uses `condition ? {} : null`. Boolean attributes " +
