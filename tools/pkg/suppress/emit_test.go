...
const (
	emptyDefaultsDatabase = `{"api_endpoint":"https://example.invalid","discovered":0,"failed":0,"generated_at":"2026-01-02T03:04:05Z","resources":{},"skipped":0,"total_resources":0,"version":"test"}`
	validDefaultsDatabase = `{"api_endpoint":"https://example.invalid","discovered":1,"failed":0,"generated_at":"2026-01-02T03:04:05Z","resources":{"probe":{"category":"simple","defaults":{"spec.flag":{"default_value":false,"path":"spec.flag","type":"bool"}},"discovered_at":"2026-01-02T03:04:05Z","request_sent":{"metadata":{},"spec":{}},"resource_name":"probe","response_got":{"metadata":{},"spec":{}},"status":"discovered"}},"skipped":0,"total_resources":1,"version":"test"}`
)
...
func TestEmitImportSuppressionsRejectsMalformedDefaultsDatabase(t *testing.T) {
	tests := []struct {
		...
		{name: "blank api_endpoint", mutate: func(db map[string]interface{}) { db["api_endpoint"] = "" }},
		{name: "relative api_endpoint", mutate: func(db map[string]interface{}) { db["api_endpoint"] = "/api" }},
		...
		{name: "resource key whitespace", mutate: func(db map[string]interface{}) {
			resource := resourceFixture(db)
			delete(db["resources"].(map[string]interface{}), "probe")
			resource["resource_name"] = "bad name"
			db["resources"].(map[string]interface{})["bad name"] = resource
		}},
		{name: "resource name mismatch", mutate: func(db map[string]interface{}) { resourceFixture(db)["resource_name"] = "other" }},
		...
		{name: "blank discovered_at", mutate: func(db map[string]interface{}) { resourceFixture(db)["discovered_at"] = " " }},
		{name: "invalid discovered_at", mutate: func(db map[string]interface{}) { resourceFixture(db)["discovered_at"] = "today" }},
		{name: "empty request object", mutate: func(db map[string]interface{}) { resourceFixture(db)["request_sent"] = map[string]interface{}{} }},
		{name: "null response object", mutate: func(db map[string]interface{}) { resourceFixture(db)["response_got"] = nil }},
		...
		{name: "blank failed error", mutate: makeStatusMutation("failed", "error", " ")},
		{name: "blank skipped reason", mutate: makeStatusMutation("skipped", "skip_reason", "")},
	}
...
func TestEmitImportSuppressionsAcceptsEveryDiscoveryStatus(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "failed", mutate: makeStatusMutation("failed", "error", "synthetic failure")},
		{name: "skipped", mutate: makeStatusMutation("skipped", "skip_reason", "synthetic skip")},
	} {
		t.Run(test.name, func(t *testing.T) {
			root := t.TempDir()
			databasePath := filepath.Join(root, "api-defaults.json")
...
func TestEmitImportSuppressionsCrossProcessMerge(t *testing.T) {
	...
	for i := range children {
		database := decodeDatabaseFixture(t)
		resources := database["resources"].(map[string]interface{})
		resource := resources["probe"].(map[string]interface{})
		delete(resources, "probe")
...
func resourceFixture(database map[string]interface{}) map[string]interface{} {
	return database["resources"].(map[string]interface{})["probe"].(map[string]interface{})
}
...
func makeStatusMutation(status, detailField, detail string) func(map[string]interface{}) {
	return func(database map[string]interface{}) {
		resource := resourceFixture(database)
		resource["status"] = status
		resource[detailField] = detail
		delete(resource, "defaults")
		delete(resource, "discovered_at")
		delete(resource, "response_got")
		if status == "skipped" {
			delete(resource, "request_sent")
		}
